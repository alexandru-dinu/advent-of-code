...
def all_transforms(tile: Tile) -> Set[Tile]:
    f = tile.fliplr()
    return [
        *[tile.rot(k) for k in range(4)],
        *[f.rot(k) for k in range(4)]
    ]
...
def assemble(tiles: Dict[int, Tile]) -> np.ndarray:
    ...
    def _inner(rest: Set[int], acc: GridT) -> bool:
        ...
        is_valid = lambda tile: \
            ((i == 0) or (tile.top == acc[i-1, j].bottom).all()) and \
            ((j == 0) or (tile.left == acc[i, j-1].right).all())

        for tile in filter(is_valid, chain(*[all_transforms(tiles[tid]) for tid in rest])):
            acc[i, j] = tile

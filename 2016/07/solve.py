...
if _coconut.typing.TYPE_CHECKING:  #1 (line in Coconut source)
    from typing import TextIO  #1 (line in Coconut source)
else:  #1 (line in Coconut source)
    try:  #1 (line in Coconut source)
        TextIO = _coconut.typing.TextIO  #1 (line in Coconut source)
    except _coconut.AttributeError as _coconut_imp_err:  #1 (line in Coconut source)
        raise _coconut.ImportError(_coconut.str(_coconut_imp_err))  #1 (line in Coconut source)
import re  #2 (line in Coconut source)

PATTERN = re.compile(r'([^\[\]]+)|\[([\w+]+)\]')  #4 (line in Coconut source)

@_coconut_tco  #6 (line in Coconut source)
def get_inp(fp  # type: TextIO  #6 (line in Coconut source)
    ):  #6 (line in Coconut source)
    return _coconut_tail_call(fp.read().strip().split, '\n')  #6 (line in Coconut source)


def supports_abba(ip  # type: str  #8 (line in Coconut source)
    ):  #8 (line in Coconut source)
    def abba(x  # type: str  #9 (line in Coconut source)
        ):  #9 (line in Coconut source)
        return x[0] == x[-1] and x[1] == x[-2] and x[0] != x[1]  #9 (line in Coconut source)


    ret = False  #11 (line in Coconut source)

    for out, ins in (PATTERN.findall)(ip):  #13 (line in Coconut source)
        ret |= out != '' and 1 == (sum)(((abba)(out[i:i + 4]) for i in range(len(out) - 3)))  #14 (line in Coconut source)
        if ins != '' and (any)(((abba)(ins[i:i + 4]) for i in range(len(ins) - 3))):  #15 (line in Coconut source)
            return False  #16 (line in Coconut source)

    return ret  #18 (line in Coconut source)



def supports_ssl(ip  # type: str  #21 (line in Coconut source)
    ):  #21 (line in Coconut source)
    """aba outside and corresponding bab inside"""  #22 (line in Coconut source)

    @_coconut_tco  #24 (line in Coconut source)
    def explode(s):  #24 (line in Coconut source)
        return _coconut_tail_call(_coconut.set, (s[i:i + 3] for i in range(len(s) - 2)))  #24 (line in Coconut source)


    outs = set()  #26 (line in Coconut source)
    inss = set()  #27 (line in Coconut source)

    for out, ins in (PATTERN.findall)(ip):  #29 (line in Coconut source)
        ([inss, outs][out != '']).update((explode)((out or ins)))  #30 (line in Coconut source)

    for a, b, c in outs:  #32 (line in Coconut source)
        if a != c:  #33 (line in Coconut source)
            continue  #33 (line in Coconut source)
        if "{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format(_coconut_format_0=(b), _coconut_format_1=(a), _coconut_format_2=(b)) in inss:  #34 (line in Coconut source)
            return True  #35 (line in Coconut source)

    return False  #37 (line in Coconut source)



def test_example():  #40 (line in Coconut source)
    assert supports_abba("abba[mnop]qrst")  #41 (line in Coconut source)
    assert not supports_abba("abcd[bddb]xyyx")  #42 (line in Coconut source)
    assert not supports_abba("aaaa[qwer]tyui")  #43 (line in Coconut source)
    assert supports_abba("ioxxoj[asdfgh]zxcvbn")  #44 (line in Coconut source)

    assert supports_ssl("aba[bab]xyz")  #46 (line in Coconut source)
    assert not supports_ssl("xyx[xyx]xyx")  #47 (line in Coconut source)
    assert supports_ssl("aaa[kek]eke")  #48 (line in Coconut source)
    assert supports_ssl("zazbz[bzb]cdb")  #49 (line in Coconut source)



def main():  #52 (line in Coconut source)
    with open(0) as fp:  #53 (line in Coconut source)
        ips = (get_inp)(fp)  #54 (line in Coconut source)

    p1 = (sum)((_coconut_call_or_coefficient(_coconut_partial(_coconut_partial, map), supports_abba))(ips))  #56 (line in Coconut source)
    p2 = (sum)((_coconut_call_or_coefficient(_coconut_partial(_coconut_partial, map), supports_ssl))(ips))  #57 (line in Coconut source)

    print("Part 1: {_coconut_format_0}".format(_coconut_format_0=(p1)))  #59 (line in Coconut source)
    print("Part 2: {_coconut_format_0}".format(_coconut_format_0=(p2)))  #60 (line in Coconut source)

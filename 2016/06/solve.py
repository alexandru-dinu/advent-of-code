...
from pathlib import Path  #1 (line in Coconut source)
if _coconut.typing.TYPE_CHECKING:  #2 (line in Coconut source)
    from typing import TextIO  #2 (line in Coconut source)
else:  #2 (line in Coconut source)
    try:  #2 (line in Coconut source)
        TextIO = _coconut.typing.TextIO  #2 (line in Coconut source)
...
from collections import Counter  #3 (line in Coconut source)


@_coconut_tco  #6 (line in Coconut source)
def get_inp(fp  # type: TextIO  #6 (line in Coconut source)
    ):  #6 (line in Coconut source)
    return _coconut_tail_call((list), (zip)(*(fmap)(_coconut.operator.methodcaller("strip"), (fp).readlines())))  #7 (line in Coconut source)



@_coconut_tco  #10 (line in Coconut source)
def solve(xs, k):  #10 (line in Coconut source)
    return _coconut_tail_call((''.join), (fmap)(_coconut_base_compose(Counter, (_coconut.operator.methodcaller("most_common"), 0, False), (_coconut_attritemgetter(None, (False, (k)), (False, (0))), 0, False)), xs))  #11 (line in Coconut source)



def test_example():  #14 (line in Coconut source)
    with open(Path(__file__).parent / "example") as fp:  #15 (line in Coconut source)
        xs = (get_inp)(fp)  #16 (line in Coconut source)

    assert "easter" == ((solve)(xs, k=0))  #18 (line in Coconut source)
    assert "advent" == ((solve)(xs, k=-1))  #19 (line in Coconut source)



def main():  #22 (line in Coconut source)
    with open(0) as fp:  #23 (line in Coconut source)
        xs = (get_inp)(fp)  #24 (line in Coconut source)

    print("Part 1: {_coconut_format_0}".format(_coconut_format_0=((solve)(xs, k=0))))  #26 (line in Coconut source)
    print("Part 2: {_coconut_format_0}".format(_coconut_format_0=((solve)(xs, k=-1))))  #27 (line in Coconut source)



if __name__ == "__main__":  #30 (line in Coconut source)
    main()  #31 (line in Coconut source)

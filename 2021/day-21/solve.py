...
def part2_weighted(start1: int, start2: int) -> int:
    mem = {}
...
def main():
    ...
    print("-" * 32)
    print("Part 2 (weighted):", part2_weighted(s1, s2))

    p = np.zeros((10, 10))
    for s1 in range(1, 11):
        for s2 in range(1, 11):
            p[s1 - 1, s2 - 1] = part2_weighted(s1, s2)

    print(pd.DataFrame(p).to_markdown())
